...
int main() {
  ...
  if (problem.stages.size() != 2 || problem.stages[0].A.rows() != 1 ||
      ...
      problem.terminal_Q.rows() != 2 || problem.terminal_Q.cols() != 2 ||
  ...
  if (!Zero(problem.stages[0].A) || !Zero(problem.stages[0].B) ||
      !Zero(problem.stages[0].c) || !Zero(problem.stages[0].Q) ||
      !Zero(problem.stages[0].R) || !Zero(problem.stages[0].M) ||
      !Zero(problem.stages[0].q) || !Zero(problem.stages[0].r) ||
      !Zero(problem.terminal_Q) || !Zero(problem.terminal_q) ||
