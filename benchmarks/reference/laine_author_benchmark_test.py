...
def check(executable):
    ...
    for row in author:
        if None in row:
            raise AssertionError("malformed author CSV column count")
        ...
        dual_error = float(row["dual_error_inf"])
        if any(math.isnan(value) for value in (feasibility, stationarity, kkt, dual_error)):
    ...
    if not float(zero["dual_error_inf"]) > 1e-3:
        raise AssertionError("original-coordinate author dual error was hidden")
