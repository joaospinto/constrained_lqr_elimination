...
inline constexpr std::array<std::size_t, 6> kPaperStateDimensions = {
    8, 16, 24, 32, 48, 64};
...
inline std::vector<PaperCase> PaperCases(const std::string &suite) {
  ...
  if (suite == "smoke")
    return {{"state", 1, 4, 2, 0, 1},
            ...
            {"combined", 17, 8, 4, 1, 1},
            {"zero", 0, 4, 2, 0, 0}};
  // The default run uses the complete Cartesian grid.
  // Smaller sweeps must be requested explicitly, never silently substituted.
  if (suite == "all")
    for (const std::size_t n : kPaperStateDimensions)
