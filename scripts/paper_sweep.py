...
def run(command, *, manifest, backend, name, output, errors, jax=False,
    ...
    if not cases or not backend:
        raise ValueError("a nonempty case list and one backend are required")
    ...
    report(f"SWEEP START: {total} cases; backend={backend}")
...
def main():
    ...
    return run(command, manifest=args.manifest, backend=args.backend,
